...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.tree = ttk.Treeview(self, columns=('ID', 'name', 'tel', 'email','wages'),
    ...
    def update_record(self, name, tel, email, wages):
        self.db.c.execute('''UPDATE db SET name=?, tel=?, email=?, wages=? WHERE ID=?''',
...
class DB:
    def __init__(self):
        ...
        self.c.execute(
            '''CREATE TABLE IF NOT EXISTS db (id integer primary key, name text, tel text, email text, wages text)''')
        # сохранение изменений БД
        self.conn.commit()
        
    # Continue adding more data here...

    # метод добавления в БДfor data in sample_data:
    
    def insert_bulk_data(self, data):
        self.c.executemany("INSERT INTO db (name, tel, email, wages) VALUES (?, ?, ?, ?)", data)
        self.conn.commit()
    def insert_data(self, name, tel, email, wages):
        self.c.execute('''INSERT INTO db (name, tel, email, wages) VALUES (?, ?, ?, ?)''',
